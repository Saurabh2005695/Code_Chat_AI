...
import { Sparkles, ArrowRight, ShieldCheck, GitBranch, Cpu, BookOpen } from 'lucide-react';
      ...
      <header className="relative z-10 px-6 sm:px-12 py-6 flex items-center justify-between">
        <Link to="/" className="flex items-center space-x-3 group">
          <svg className="w-9 h-9 group-hover:scale-105 transition-transform" viewBox="0 0 40 40" fill="none" aria-hidden="true">
            <defs>
              <clipPath id="gclip-signup"><circle cx="20" cy="20" r="18.2"/></clipPath>
            </defs>
            <circle cx="20" cy="20" r="18.4" stroke="#0d1b30" className="dark:stroke-[#96A6C4]" strokeWidth="1.1"/>
            <g clipPath="url(#gclip-signup)" stroke="#0d1b30" className="dark:stroke-[#F0F4FA]" fill="none" strokeLinecap="round" strokeLinejoin="round">
      ...
      <main className="relative z-10 max-w-6xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center flex-1">
        ...
        <div className="lg:col-span-6 space-y-6 text-left">
          ...
          <p className="text-sm sm:text-base text-[var(--muted2)] dark:text-slate-300 leading-relaxed max-w-md">
            Create your account to index GitHub repositories, upload ZIP archives, and chat in Hinglish or English with hybrid AST search.
          </p>

          <div className="space-y-3 max-w-md pt-2">
            <div className="flex items-center space-x-3 text-xs sm:text-sm text-[var(--ink-soft)] dark:text-slate-200">
              <GitBranch className="w-4 h-4 text-[var(--accent)] flex-shrink-0" />
              <span>Multi-repo workspace with live indexing status</span>
            </div>
            <div className="flex items-center space-x-3 text-xs sm:text-sm text-[var(--ink-soft)] dark:text-slate-200">
              <Cpu className="w-4 h-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
              <span>Configurable high-speed LLM Providers (Gemini, Groq, Ollama)</span>
            </div>
            <div className="flex items-center space-x-3 text-xs sm:text-sm text-[var(--ink-soft)] dark:text-slate-200">
              <BookOpen className="w-4 h-4 text-[var(--accent)] flex-shrink-0" />
              <span>Auto-generated READMEs & Interactive Architecture Graphs</span>
            </div>
          </div>
        </div>
